...
from .forms import LoginForm, ResetPasswordKeyForm, EditProfileForm
...
from GroupInvitations.models import GroupInvitation
...
class SignupView(RedirectAuthenticatedUserMixin, CloseableSignupMixin,
    ...
    def form_valid(self, form):
        ...
        user.groups.add(Group.objects.get(name=
            GroupInvitation.objects.get(email=form.cleaned_data["email"]).group))
...
class EditProfileView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        get_adapter().add_message(self.request,
                                  messages.SUCCESS,
                                  'account/messages/profile_edited.txt')
        signals.profile_edited.send(sender=self.request.user.__class__,
                                      request=self.request,
                                      user=self.request.user)
        return super(EditProfileView, self).form_valid(form)

    def get_context_data(self, **kwargs):
        ret = super(EditProfileView, self).get_context_data(**kwargs)
        # NOTE: For backwards compatibility
        ret['edit_profile_form'] = ret.get('form')
        # (end NOTE)
        return ret
