...
from .models import EmailAddress, Profile, Address, PhoneNumber
...
class EditProfileForm(UserModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key in self.initial_fields:
            if hasattr(self.user, key):
                initial[key] = initial.get(key) or getattr(self.user, key)
        self.profile = self.user.profile
        for key in self.initial_fields:
            if hasattr(self.profile, key):
                initial[key] = initial.get(key) or getattr(self.profile, key)
